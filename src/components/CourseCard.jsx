...
const HiStar = ({ filled }) => (
  <svg width="14" height="14" fill={filled ? 'var(--warning)' : 'none'} stroke={filled ? 'var(--warning)' : 'currentColor'} strokeWidth="1.5" viewBox="0 0 24 24">
...
export default function CourseCard({ course }) {
  ...
  return (
    <motion.div
      ...
      style={{ borderRadius: 16, overflow: 'hidden', cursor: 'pointer', display: 'flex', flexDirection: 'column' }}
    ...
    >
      <div style={{ aspectRatio: '16/9', width: '100%', position: 'relative', overflow: 'hidden' }}>
        ...
        {course.price === 0 && (
          <div style={{ position: 'absolute', top: 8, right: 8, background: 'var(--success)', color: '#fff', fontSize: 9, fontWeight: 700, padding: '3px 7px', borderRadius: 6, letterSpacing: '0.05em' }}>FREE</div>
        )}
      </div>
      <div style={{ padding: 12, flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ fontSize: 9, color: 'var(--accent)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 5 }}>{course.category}</div>
        ...
        <div style={{ marginTop: 'auto' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'baseline', gap: 6 }}>
            {course.price === 0
